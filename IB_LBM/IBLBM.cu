...
int main(int nargs, char** args )
{
  ...
  int n_particles = (*h_Pa).n_particles;	
  int nx = (*h_Pa).nx;
  int ny = (*h_Pa).ny;
  int nz = (*h_Pa).nz;
  ...
  //----
  int thread = 16;
  int block = 1 + (n_particles - 1)/thread;
  //----
  ...
  size_t Size = (n_particles)*sizeof(REAL);
  REAL *gpu_f0;
  ...
  //----
  REAL *gpu_weight;
  cudaMalloc((void**)&gpu_weight, ArraySize);
  //----
  ...
  cudaMalloc((void**)&gpu_Gx, ArraySize); //fluid force (x-component)
  cudaMalloc((void**)&gpu_Gy, ArraySize); //fluid force (y-component)
  //-----
  //ibm
  REAL *gpu_x, *gpu_y ;
  REAL *gpu_xi, *gpu_yi ;
  REAL *gpu_fx, *gpu_fy ;
  REAL *gpu_velx, *gpu_vely;
  REAL *gpu_Utotal;
  cudaMalloc((void**)&gpu_x, Size); //IBM marker position(x-component)
  cudaMalloc((void**)&gpu_y, Size); //IBM marker position(y-component)
  cudaMalloc((void**)&gpu_xi, Size); //IBM marker reference position(x-component)
  cudaMalloc((void**)&gpu_yi, Size); //IBM marker reference position(y-component)
  cudaMalloc((void**)&gpu_fx, Size); //IBM marker force(x-component)
  cudaMalloc((void**)&gpu_fy, Size); //IBM marker force(y-component)
  cudaMalloc((void**)&gpu_velx, Size); //IBM marker velocity(x-component)
  cudaMalloc((void**)&gpu_vely, Size); //IBM marker velocity(y-component)
  cudaMalloc((void**)&gpu_Utotal, sizeof(REAL)); //IBM marker total energy
  //=====CPU===
  ...
  //-----
  REAL *h_Gx = (REAL*) malloc(ArraySize); // host, cpu
  REAL *h_Gy = (REAL*) malloc(ArraySize); // host, cpu
  REAL *h_fx = (REAL*) malloc(Size);
  REAL *h_fy = (REAL*) malloc(Size);
  REAL *h_velx = (REAL*) malloc(Size);
  REAL *h_vely = (REAL*) malloc(Size);
  //-----
  REAL *h_weight = (REAL*) malloc(ArraySize);
  ...
  //---
  SetArrayValues<<<Blocks_LBM,Threads_LBM>>>(gpu_Pa,gpu_weight, 0.);
  //---
  //ibm
  SetValues<<<block,thread>>>(gpu_Pa,gpu_fx, 0);
  SetValues<<<block,thread>>>(gpu_Pa,gpu_fy, 0);
  ...
  gpuErrchk(cudaMemcpy(h_weight, gpu_weight, ArraySize, cudaMemcpyDeviceToHost));
  //----

  DUMP_LAMMPS_STYLE_DATA(h_Pa,time,frame_num,
			 h_rho, h_u, h_v,
			 h_f0, h_weight, h_x ,h_y);// time = 0, frame_num  = 0 
  //-----
  gpuErrchk(cudaMemcpy(gpu_x, h_x, Size, cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpy(gpu_y, h_y, Size, cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpy(gpu_xi, h_xi, Size, cudaMemcpyHostToDevice));
  gpuErrchk(cudaMemcpy(gpu_yi, h_yi, Size, cudaMemcpyHostToDevice));

  frame_num = 1;
  //===== main loop =====
  for (int iter = 1; iter <= iter_max; iter++){
    //if(iter >= iter_max*0.5){
    Force_Variational<<<block,thread>>>(gpu_Pa, 
               gpu_x, gpu_y, gpu_xi, gpu_yi, 
               gpu_Utotal, gpu_fx, gpu_fy);// use lattice spring model to calculate IBM force

    clear_force<<<Blocks_LBM,Threads_LBM>>>(gpu_Pa, gpu_Gx, gpu_Gy);

    cudaMemcpy(h_fx, gpu_fx, Size, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_fy, gpu_fy, Size, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_Gx, gpu_Gx, ArraySize, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_Gy, gpu_Gy, ArraySize, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_x, gpu_x, Size, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_y, gpu_y, Size, cudaMemcpyDeviceToHost);

    spread(h_Pa, h_x, h_y, h_fx, h_fy, h_Gx, h_Gy); //IBM boundary force spread to LBM

    cudaMemcpy(gpu_Gx, h_Gx, ArraySize, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_Gy, h_Gy, ArraySize, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_fx, h_fx, Size, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_fy, h_fy, Size, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_x, h_x, Size, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_y, h_y, Size, cudaMemcpyHostToDevice);
    //}
    ...
    Update_Collision<<<Blocks_LBM,Threads_LBM>>>(gpu_f0,
						   ...
						   gpu_rho,gpu_u, gpu_v,gpu_Gx, gpu_Gy, gpu_Pa); //collision step
    ...
    //===COMPUTE RHO U AND V 
    COMPUTE_RUV<<<Blocks_LBM,Threads_LBM>>>(gpu_Pa,
			    gpu_f0, gpu_f1, gpu_f2, gpu_f3, gpu_f4,
			    gpu_f5, gpu_f6, gpu_f7, gpu_f8,
			    gpu_rho, gpu_u, gpu_v, gpu_Gx, gpu_Gy); //update fluid density and velocity

    //if(iter >= iter_max*0.5){
    clear_vel<<<block,thread>>>(gpu_Pa, gpu_velx, gpu_vely); 

    cudaMemcpy(h_u, gpu_u, ArraySize, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_v, gpu_v, ArraySize, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_velx, gpu_velx, Size, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_vely, gpu_vely, Size, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_x, gpu_x, Size, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_y, gpu_y, Size, cudaMemcpyDeviceToHost);  

    interpolate(h_Pa, h_x, h_y, h_u, h_v, h_velx, h_vely);

    Weight_test(h_Pa , h_weight, h_x, h_y);

    cudaMemcpy(gpu_velx, h_velx, Size, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_vely, h_vely, Size, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_x, h_x, Size, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_y, h_y, Size, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_u, h_u, ArraySize, cudaMemcpyHostToDevice);
    cudaMemcpy(gpu_v, h_v, ArraySize, cudaMemcpyHostToDevice);
      
    update_particle_position<<<block,thread>>>(gpu_Pa , gpu_x, gpu_y, gpu_velx, gpu_vely);
    //}      
    ...
    if  ((iter % dump_frequency) == 0){
	    ...
	    gpuErrchk(cudaMemcpy(h_v, gpu_v,ArraySize, cudaMemcpyDeviceToHost));
	    gpuErrchk(cudaMemcpy(h_f0, gpu_f0,ArraySize, cudaMemcpyDeviceToHost));
      gpuErrchk(cudaMemcpy(h_x, gpu_x,Size, cudaMemcpyDeviceToHost));
	    gpuErrchk(cudaMemcpy(h_y, gpu_y,Size, cudaMemcpyDeviceToHost));
      //----
      if(iter >= iter_max*0.99){
        DUMP_LAMMPS_STYLE_DATA(h_Pa,time,frame_num, h_rho, h_u, h_v, h_f0, h_weight, h_x, h_y);
      }
      if(iter >= iter_max*0.5){
        DUMP_DATA(h_Pa,time,frame_num, h_u, h_x, h_y);
      }
	    frame_num++;
	  }
  }
